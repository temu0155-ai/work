...
const HORDE_API_KEY = process.env.HORDE_API_KEY || '0000000000'; // 10 zeros = anonymous
const HORDE_MODEL = process.env.HORDE_MODEL || '';              // empty = any available worker
  ...
  while (Date.now() < deadline) {
    ...
    try {
      statusRes = await fetch(`${HORDE_BASE}/generate/text/status/${id}`, {
        headers: { apikey: HORDE_API_KEY, 'Client-Agent': CLIENT_AGENT },
        signal: AbortSignal.timeout(15000),
      });
    } catch {
      continue; // poll blip / timeout -> retry next tick
    }
...
// ===================== ENTRYPOINT =====================
async function runAiSetup(prompt, guild, sessionId, requesterMember, onProgress = null) {
  ...
  try {
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      progress({ phase: 'thinking', round: round + 1, maxRounds: MAX_TOOL_ROUNDS });
      ...
      for (const call of calls) {
        ...
        progress({ phase: 'tool', toolName: call.name, message: toolResultText, round: round + 1, maxRounds: MAX_TOOL_ROUNDS });
...
module.exports = { runAiSetup };      const result = await runAiSetup(
        prompt,
        interaction.guild,
        sessionId,
        interaction.member,
