...
// message/voice stat counters. Returns { leveledUp, oldLevel, level, xp }
// so callers (message/voice trackers) can announce level-ups and know the
// full range of levels crossed (a single big XP grant can skip several
// levels at once).
async function applyXp(guildId, userId, xpAmount, { messageDelta = 0, voiceMinutesDelta = 0, lastMessageAt } = {}) {
  ...
  return { leveledUp, oldLevel: startLevel, level, xp, xpForNext: required };
}
